...
| 拖到 `TaskTreeItem` 行上（不分同/跨文件夹）退化成"追加到被拖条目自己所在文件夹的末尾" | `handleDrop` 的落点类型判断只认 `EntryTreeItem`/`FolderTreeItem` 两种，`TaskTreeItem` 落不到 `targetCwd`/`targetId`，于是统一走"落到空白处"的分支。没有数据错位风险（条目绝不会跑到错的文件夹），只是视觉上"看起来往那一行拖了，实际效果是插到自己文件夹末尾"，容易让人一开始以为是 bug。已知取舍，不打算专门处理——真要精确化，做法是把 `TaskTreeItem` 落点当成它父级 `EntryTreeItem` 处理即可，留作后续小改动 |

## 11. 实施后修订

实施与本设计完全吻合，无需修订；上面这条"已知取舍"是实施中（Task 3/Task 7 的
任务评审、以及最终整体评审）发现并确认的既有设计行为，不是实施偏差。
