...
| 2b | **0.2.1 修订**：用户实测反馈一级文件夹行尾的 `+` 也不该再问工作路径（即 §3.2 判断 B 里「cwd 允许改」那半句被否决） | 新增 `addEntryInFolderInteractive(cwd)`：只问名称与是否参与全部恢复，cwd **原样继承**那一行 | §7.4 §14.6 |
| 2c | **0.2.2 修订**：用户实测反馈「切模型/切 profile 几乎每次都被拒绝，即使点的是健康的三级会话」——根因是 `applyModel`/`applyProfile` 对「存活槽」（tmux 会话还在）做的是「任一个前台不是 claude 就整体拒绝」的全有全无判断，而「会话存活」≠「claude 还在跑」 | 改成**跳过**前台不是 claude 的槽（不算失败），只处理真正在跑 claude 的槽；一个健康槽都没有时才维持整体拒绝；拒绝/跳过的提示都必须点名「第几个会话」 | §7.5.1 |
| 2d | **0.2.3 修订**：用户实测批量切 profile 6 条 3 成功 3 失败，其中 2 条（专家模式/原型机）只有 1 个会话且那个会话前台是 bash——2c 的「一个健康槽都没有时才维持整体拒绝」这条本身考虑不周：没有 claude 在跑，就不存在「看起来还在用旧配置」的假象要担心 | 改成跟 `alive.length === 0`（tmux 会话都不存在）同一分支处理：直接落配置，下次这些会话重启时生效，返回 `true`；`restartClaude` 内部 `canSendControl` 的竞态兜底（发送前最后一道闸门）不受影响，仍然 `refuse` | §7.5.1 |
...
所以一级的「+」= **在该文件夹的 cwd 下新建一个二级条目**。

- 理由：文件夹不是实体，无法「在它下面」存任何东西；能做的只有把它的 cwd
  当成新建条目的默认值。**这半句仍然成立。**
- ⚠ **「cwd 预填、允许改」这半句已被用户复核并否决（0.2.1 起已改）**：用户
  实测后反馈不该再问工作路径 —— 一级不是实体，它的 cwd 就是那一行的事实，
  再问一遍等于让同一个值在两个地方维护。现在 cwd **原样继承**那一行的路径
  （不归一化、不展开 `~`），只问名称与是否参与全部恢复。
- 实现上因此从「与标题栏 `add` 共用 `addEntryInteractive(cwd?)`」**拆成了两条
  路径**（§7.4）：一级是 `addEntryInFolderInteractive(cwd)`，标题栏是
  `addEntryInteractive()`。根因与修法见 §14.6。
...
**三个入口是三条路径**（0.2.1 起。此前一级 `+` 与标题栏 `+` 共用
`addEntryInteractive`，于是点一级的 `+` 会多问一次工作路径 —— 那正是用户实测
反馈的症状，见 §14.6）：

| 入口 | 落点 | 问什么 |
|---|---|---|
| 一级 inline `+` | `addEntryInFolderInteractive(cwd)` | 名称 → 是否参与全部恢复。**cwd 不问**，原样继承那一行的路径（不归一化、不展开 `~`） |
| 标题栏 `+` | `addEntryInteractive()` | 名称 → 目录 → 是否参与全部恢复（**不变**：标题栏没有可继承的文件夹） |
| 二级 inline `+` | `addSessionInteractive(entry)` | **零弹框**（需求 2） |

```ts
// 标题栏 +：没有可继承的目录，只能问
async addEntryInteractive(): Promise<void>
// 一级 inline +：cwd 来自那一行的文件夹，原样继承
async addEntryInFolderInteractive(cwd: string): Promise<void>
```

两条条目路径**只有「问不问目录」这一处不同**，其余逐字相同：

1. 问名称（沿用 `askName`，校验重名）——**取消则整条中止**（用 `undefined`
   判断，不能用 `!name`，理由见 `terminalManager.ts:1097-1100`）。
2. **只有标题栏 `+` 问目录**（沿用 `askCwd`）。一级 `+` 跳过这一步：cwd 原样
   取那一行的路径，**不归一化、不展开 `~`** —— 继承就是继承，值不该在继承的
   路上被改写。
...
```ts
// 在锁内追加一个槽。**conversationId 必须预分配**，理由见下面那段 ——
// 它与「不弹框」是同一件事的两面。
...
**⚠ 新槽必须一出生就带一个「还不存在对应 .jsonl」的 `conversationId`，
不能留空。** 这不是风格问题，是**「点开就是一个全新对话」与「悄悄接到别人的
对话上」的分界**：

- `resolveLaunchSpec` 只在**已绑定**分支里才会返回 `fresh`（裸 `claude`）。
  它的判据是「有 `conversationId`、但 `findConversations` 查不到任何 .jsonl」
  —— 那正是「刚分配、还没用过」的形态，于是走 `fresh`（且因为
  `liveSessionId` 也是空的，连那句「对话记录似乎已被删除」的提示都不会弹）。
- **留空则落进未绑定路径 → 兜底 D**：该 cwd 下只有一个条目时，它会**静默**
  `--resume` 该目录下 mtime 最新的那条对话。用户「+ 然后点开」期待的是开一条
  新对话，实际却接到了别的会话上；而在多会话模型下，那条最新的对话**很可能
  正是同一终端里另一个槽正在写的**（见 §11.16）—— 两个 claude 同写一条
  `.jsonl`，数据损坏级。
- 这与 `addEntryInteractive`（§7.4 上面）**必须一致**：新建条目的槽也是一出生
  就分配 `conversationId`（`0.1.8` 起就是这么做的，注释写着「新建的条目永远
  不会被弹选择框」）。两个新建入口走同一条路径，不能一个分配、一个留空。

其余：无 `liveSessionId`、`order` 在锁内分配（`nextSessionOrder`）。
...
### 7.5.1 **0.2.2 修订**：「转发到整个条目」≠「一个不合格就整体拒绝」

`setModelInteractive`/`setProfileInteractive` 落到 `applyModel`/
`applyProfile` 后，作用范围仍是**整个条目下所有存活的槽**（本条不变，
上面 7.5 的表格与不变量 6 都还成立）——变的只是**「存活槽里混进一个不是
claude 的槽时该怎么办」**。

**旧行为（bug）**：对 `aliveSlots(entry)`（tmux 会话还在的槽）逐个跑
`canSendControl`，**任一个前台不是 claude 就整体 `refuse`、一个都不改**。
`aliveSlots` 只代表「tmux 会话还在」，不代表「claude 还在跑」——用户在
某个槽里 `/exit` 或 claude 崩溃，都会让会话活着但前台变回 `bash`。三级
点击按 7.5 转发到整个条目，于是「点的明明是健康的第 3 个会话，却因为
**兄弟**会话已经退回 bash 而被整体拒绝」；错误提示又只报 `entry.name`，
用户看到的是「点了三级，却说『二级标题』不是 claude」，无法定位问题。
实测这台机器上三个条目、每个都恰好一个槽已经退回 bash，导致这三个条目
的切模型/切 profile **100% 复现**。

**新行为（用户已确认的产品决策，只改这一条，不动 7.5 的转发语义）**：

1. 对存活槽按「前台是不是 claude」分成 `eligible`（真正在跑 claude）与
   `skipped`（前台不是 claude，如已回落 bash）两组。
2. `skipped` 里的槽**不发送任何命令、不重启、不计入失败**——只是跳过。
3. 只处理 `eligible` 里的槽；处理完（`applyModel` 发完 `/model`、
   `applyProfile` 重启完这些槽）照常落盘 `profile`/`model` 配置。
4. **`eligible.length === 0`**（存活槽存在，但没有一个在跑 claude）时，
   与「全都不合格」等价，维持**整体拒绝、不落配置**——这与「跳过一部分」
   是两种不同的结果，不能混。
5. `applyProfile` 额外区分「跳过」与「真失败」：若某个 `eligible` 槽自己
   的 `restartClaude` 失败（例如没有绑定对话、重启后没等到 shell），这是
   **真正尝试过、失败了**，与「因前台不是 claude 而从未尝试」不同——仍按
   原有的保守语义处理（中途失败整体不落配置，因为已重启的槽已经在用新
   配置连接了，回滚不了）。
6. 任何一次「有槽被跳过」都必须弹提示、**点名第几个会话**、说明原因是
   「未在跑 claude」——不能像旧版那样只报条目名，让用户以为整个操作对象
   搞错了。全部健康、没有槽被跳过时保持原样安静成功，不额外弹窗。

批量套用（`applyModelToMany`/`applyProfileToMany` → `applyToMany`）不需要
额外改动：它只看 `applyModel`/`applyProfile` 的布尔返回值统计「成功/失败」，
而「有槽被跳过但整体仍成功」现在如实返回 `true`，批量汇报的「成功 N」
count 因此不会把「有槽被跳过」误报成「这个条目切换失败了」——跳过的细节
由条目自己那条 info 提示承担，批量汇报不需要重复它。
...
3. **`resolveLaunchSpec` 的绑定来源从 `entry.conversationId` 换成
   ...
   在别的目录 → 报错不接，未绑定 → 弹一次选择框）全部照旧。
   **兜底 D 多了一条判据（候选不得被任何槽占用），见 §11.16 —— 那不是「照旧」，
   是多会话模型必须补的洞。**
...
16. **兜底 D 必须同时满足两条**（§8 第 3 点、§11.16 本条）：
    **(a) 该 cwd 下只有这一个条目**（判据按 `cwdFor()` **展开后**的路径比较，
    `~/x` 与 `/home/u/x` 算同一目录）；
    **(b) 它挑中的那个候选对话，当前不被任何槽绑定**（含本条目下的其它槽）。

    > **(b) 是本轮新增的，且不可省 —— 它是多会话模型引入的一个新洞。**
    > D 原本的正当性论证是「单条目 → 不可能与别的条目争同一条 .jsonl」。
    > **这个论证在多会话下失效了：竞争者可以来自同一个条目。**
    >
    > 可达路径（实测推演）：条目 E（cwd=X）有一个**未绑定**的槽 s1
    > （v1 迁移来的、或 v2 里本来就没有 `conversationId` 的老条目），用户点
    > 「+」加了 s2（预分配了 `conversationId`）并点开 —— s2 正在写对话 C。
    > 此时点 s1：条目数 = 1，(a) 成立 → D 生效 → `candidatesForCwd` 取 mtime
    > 最新的候选，**而 C 正是最新的那条** → s1 也 `--resume C`。
    > **两个 claude 同写一条 .jsonl**，正是本仓库从 `--continue` 那条注释起
    > 一路在防的数据损坏。
    >
    > 判据用现成的 `ownersOf`（`core/conversation.ts:283`，它本就是为回答
    > 「这条对话归谁」而存在的）：把**所有条目的所有槽**摊平成
    > `BindingView[]`，取第一个**无主**的候选。有主则**不启用 D**，退回
    > 弹框让用户自己决定 —— 与「手动改绑别人的对话要二次确认」同一侧。
    >
    > (a) 仍然保留：它挡的是「多个条目争抢、谁都别自动猜」；(b) 挡的是
    > 「同一个条目内部自己跟自己抢」。两条判的是不同的东西，缺一不可。

17. **凡是「这条对话归谁」的判据，摊平出来的视角 id 一律用槽 id，不用条目 id。**

    `ownersOf`（`core/conversation.ts:283`）的「跳过自己」**只按传入的 id 生效**。
    多会话下**同一个条目下的两个槽可以争同一条对话**（用户手动选、或手改清单
    文件），所以按**条目 id** 跳过自己会把兄弟槽**全部**跳掉 —— 不报错，只是
    「已绑给「X」」标记与二次确认**静默消失**，用户于是能把第二个槽也绑到兄弟槽
    正在写的那条对话上：**两个 claude 同写一条 `.jsonl`**。

    这是多会话模型打破的一个隐含假设：**「一个条目 = 一个行为主体，条目内部
    不可能自己跟自己抢」**。该假设在 v2 成立（一条目一会话），在 v3 **不成立**。
    它一共在三个地方被依赖过，三处都要按槽 id 摊平：

    | 落点 | 状态 |
    |---|---|
    | 兜底 D 的判据 (b)（§11.16） | 已按槽 id 摊平 |
    | `pickConversation` 的归属标记 + 二次确认 | **必须改**（见 §11.17 本条） |
    | `bindConversationInteractive`（走同一个 `pickConversation`） | 随之覆盖 |

    条目名仍照旧传 `name`（归属标签要显示的是**终端名**），只有**视角 id**
    这一位要换成槽 id。
...
| **兜底 D 收紧后，老条目可能比今天多弹一次选择框** | 新增的判据 (b)（候选不得被任何槽占用，§11.16）意味着：某 cwd 下唯一的老条目，若该目录里最新的对话恰好已被别的槽绑着，D 不再自动采用它，而是弹框问。代价是偶尔多一次交互；换来的是堵住「同一条目内两个槽 `--resume` 同一条 .jsonl」这条静默的数据损坏路径。按本仓库一贯取舍（宁可多问一次，绝不猜），这个方向是对的。 |
...
### 14.4 测试基建缺陷：harness 在中断路径上**必然**漏夹具（**已修**：`6ba4400`）

> **状态（2026-09-16）**：本条描述的缺陷**已经修掉**，修法就是下面写的结构性
> 补救 —— 在 `test/e2e-harness.js` 里加进程级同步兜底清扫。下面保留完整的
> 现象与根因，作为该修法的依据；**读代码时以仓库现状为准，不要以为它还在**。
>
> 已验证（`6ba4400`）：造一个**用户形态**（12 位十六进制、且以 `e2e` 开头
> —— 即最容易误杀的碰撞形态）与一个**夹具形态**的会话，中途 SIGTERM 掉
> harness → 夹具被扫掉、用户形态那个**活着**；判据 11/11 分类正确。
> 全绿那轮跑完夹具残留为 **0**。
...
### 14.6 0.2.1：用户实测两条（颜色候选认不出颜色 / 一级 `+` 多问一次目录）

**先说一句：用户报的「点二级标题的 `+` 新建会话时还在问工作路径」，在当前代码
上复现不出来。** 用真 `activate()` + 真命令处理器做的探针证明：二级 `+`
（`tmuxTerminals.addSession` → `addSessionInteractive(entry)`）**从来就是零弹框**
—— 选择框 / 输入框 / 提示框一个都不弹，这条在 e2e §24a 也一直钉着。真正会问工作
路径的是**一级文件夹行尾**的 `+`（下表缺陷 1）。如实记在这里，免得后人照着这个
症状去找一个不存在的回归。

| # | 问题 | 根因 | 修法 |
|---|---|---|---|
| 1 | 一级文件夹行尾 `+` 会问一次「目录」 | **两条路径共用一个 helper**：`addInFolder` 与标题栏的 `add` 都走 `addEntryInteractive`，而它的第 2 步就是 `askCwd`。一级 `+` 的 cwd 本就有确定来源（那一行），再问一遍是重复劳动 | 拆出 `addEntryInFolderInteractive(cwd)`：只问名称与是否参与全部恢复，cwd **原样继承**（§7.4）。**标题栏那条不受影响** —— 它没有可继承的文件夹，目录只能问 |
| 2 | 「设置颜色…」的候选项只有一串 `#rrggbb` 文本，认不出那是什么颜色 | 候选项此前只给 `label` | 每项加 `QuickPickItem.iconPath` = **16×16 实心圆角色块**；hex 文本**同时保留**（只给色块没法手抄、没法比对，两个都要） |

**色块为什么不复用二级行首那条竖线**：竖线是 `colorBarSvg` 画的
`<rect width="2" height="12">`（§6.4），2px 宽的东西放进选择框里小得**看不出
颜色** —— 而「一眼认出这是什么颜色」正是这次要解决的问题。故另画一个圆角色块，
复用同一个 `ColorIconCache`、同一个 `<globalStorage>/colors` 目录，只把文件名
分开：色块是 `<hex>.swatch.svg`，竖线还是 `<hex>.svg`。

三条实现纪律，每条都对应一个会**静默**出错的顺序或排布：

- **先落盘再弹框**：`iconPath` 只拼路径、不读盘（§6.4 的「渲染路径零 IO」），
  文件没到位就是一个**空图标**，而且**不报错** —— 弹框是同步的、落盘是异步的，
  顺序反了用户看到的是一排空白项，还查不出来。
- **当前色本身是预设时保持原序**：只在原位标记（`description: '当前'` +
  `picked: true`），**顺序不随当前色跳动** —— 否则同一份调色板每打开一次就换
  一次位置，用户的位置记忆全废。
- **当前色是自定义 hex 时插到首位**：它不在预设里，没有「原位」可标；排到最前
  才能让「我现在用的是哪个」一眼可见，也让「再选同一个值」不必去末项输入框里
  重敲。末项仍是「$(pencil) 自定义…」，走输入框。

**测试缺口（本批的元教训）**：`extension.ts` 里 `registerCommand(id, fn)` 的
**接线** —— 命令 ID 到底接到了哪个方法上 —— 此前**没有任何用例跑过**：所有用例
都是直接调 `manager.xxx()`，唯一沾边的是 `test/manifest.test.ts` 的静态一致性
检查，而它只管「声明过的都注册了」。**静态检查证明不了接线接对**：把二级 `+`
接到会问目录的那条路径上，静态检查一样全绿 —— 而用户的症状恰好出在接线层。

新增 e2e 第 32 节补上这层覆盖：真跑一次 `activate()`、从 stub 捕获命令处理器、
再拿真命令驱动它，覆盖三条 —— 二级 `+` 零弹框；一级 `+` 不问目录且继承 cwd；
「设置颜色」的候选项带色块 `iconPath` + 当前色标记 + 文件已落盘。

这与 §14.1 第 4 条（`migrateAndBackupV2` 写好了却没人调用）是**同族**的元教训：
**「接线」这一层既不在纯函数单测的射程内，也不在静态一致性检查的射程内** ——
只有「捕获真实注册的处理器、再驱动它」这种用例盖得住。
